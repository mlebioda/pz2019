...
module.exports = function(url, req, rep, query, payload, session) {
    ...
    switch(url) {
        ...
        case '/defined':
            switch (req.method) {
                case 'GET':
                    const account_id = mongodb.ObjectId(common.sessions[session].accountNo);
                    common.defined.aggregate([
                        {$match: {account: account_id}},
                        {$lookup:{from:'accounts',localField:'account',foreignField:'_id',as:'account'}},
                        {$unwind:{path:'$account'}},
                        {$addFields:{my_email:'$account.email'}},
                        {$project:{account:false}},
                    ]).toArray(function(err, entries) {
                        if(err) {
                            lib.sendJSONWithError(rep, 400, 'transfers retrieving failed');
                        }
                        else {
                            lib.sendJSON(rep, entries);
                        }
                    ...
                    break;
                case 'POST':
                    common.accounts.find({email: payload.email}).toArray(function(err, recipient) {
                        if(err || recipient.length != 1) {
                            lib.sendJSONWithError(rep, 400, 'Recipient unknown or ambiguous');
                            return;
                        }
                    if(!payload || !payload.email || !payload.amount || !payload.description) {
                        lib.sendJSONWithError(rep, 401, 'Invalid data');
                        return;
                    }
                    else {
                        let id = mongodb.ObjectId(common.sessions[session].accountNo);
                        common.accounts.findOne({_id: id}, function (err ,account) {
                            if(err) {
                                lib.sendJSONWithError(rep, 400, 'Błędny użytkownik');
                            } else {
                                common.accounts.findOne({email: payload.email}, function (err, recipient) {
                                    if(err) {
                                        lib.sendJSONWithError(rep, 400, 'Brak odbiorcy w bazie');
                                    } else {
                                       common.defined.insertOne({
                                           account: account._id,
                                           recipient: recipient.email,
                                           amount: payload.amount,
                                           description: payload.description
                                       });
                                       lib.sendJSON(rep, {message: "Utworzono przelew zdefiniowany"});
                                    }
                                })
                            }
                        })
            ...
            case '/definedupdate':
                switch (req.method) {
                    case 'POST':
                            common.accounts.find({email: payload.email}).toArray(function(err, recipient) {
                                ...
                                common.defined.updateOne(
                                    { _id: mongodb.ObjectId(payload._id) },
                                    {
                                   ...
                                   },function(err, recipient)
                                   {
                                       if(err)
                                       {
                                           lib.sendJSONWithError(rep, 400, 'Zaktualizowano');
                                       }
                                       else{
                                          if(isNaN(payload.amount) || payload.amount <= 0) {
                                              lib.sendJSONWithError(rep, 400, 'Podana kwota musi być większa niż 0');
                                          }
                                          else if(!payload.description || !payload.email)
                                          {
                                              lib.sendJSONWithError(rep, 400, 'Pola muszą być wypełnione');
                                          }
                                          else
                                          {
                                           lib.sendJSON(rep, {message:"Zaktualizowano"});
                                          }
                                       }
                                   });
                                   return;
             ...
             case '/defineddelete':
                switch (req.method) {
                    case 'POST':
                        console.log(payload);
                        let id = mongodb.ObjectId(payload._id);
                    common.defined.deleteOne({_id: id}, function(err, recipient)
                    {
                        if(err)
                        {
                            lib.sendJSONWithError(rep, 400, 'Nie usunięto');
                        }
                        else{
                            lib.sendJSON(rep, {message:"Usunięto"});
                        }
                        return;
                    });
                    return;
